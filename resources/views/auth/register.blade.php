...
@section('title', __('auth_create_account') . ' — WebCheckApp')
@section('meta_description', __('auth_create_desc'))
...
<div class="max-w-md mx-auto px-4 py-16">
    ...
    <div class="text-center mb-10">
        <h1 class="text-3xl font-bold mb-2">{{ __('auth_create_account') }}</h1>
        <p class="text-gray-400">{{ __('auth_create_desc') }}</p>
    </div>
    ...
    <div class="bg-white/3 border border-white/8 rounded-2xl p-8">
        ...
        <a href="{{ route('auth.google') }}"
           class="w-full flex items-center justify-center gap-3 bg-white/5 border border-white/10 hover:bg-white/10 text-white font-semibold py-3 rounded-xl transition mb-6">
            ...
            {{ __('auth_signup_google') }}
        </a>

        <div class="flex items-center gap-4 mb-6">
            <div class="flex-1 border-t border-white/10"></div>
            <span class="text-xs text-gray-500 uppercase">{{ __('auth_or') }}</span>
            <div class="flex-1 border-t border-white/10"></div>
        </div>
        ...
        <form action="{{ route('register') }}" method="POST" class="space-y-5">
            ...
            <div>
                <label class="block text-sm text-gray-400 mb-1.5" for="name">{{ __('auth_name') }}</label>
                <input type="text" id="name" name="name" value="{{ old('name') }}" required autofocus
                       ...
                       placeholder="{{ __('auth_name_placeholder') }}">
            </div>

            <div>
                <label class="block text-sm text-gray-400 mb-1.5" for="email">{{ __('auth_email') }}</label>
            ...
            <div>
                <label class="block text-sm text-gray-400 mb-1.5" for="password">{{ __('auth_password') }}</label>
                <input type="password" id="password" name="password" required
                       class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
                       placeholder="{{ __('auth_password_min') }}">
            </div>

            <div>
                <label class="block text-sm text-gray-400 mb-1.5" for="password_confirmation">{{ __('auth_confirm_password') }}</label>
                <input type="password" id="password_confirmation" name="password_confirmation" required
                       class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
                       placeholder="{{ __('auth_confirm_placeholder') }}">
            </div>
        ...
        <p class="text-center text-sm text-gray-500 mt-6">
            {{ __('auth_already_account') }}
            <a href="{{ route('login') }}" class="text-indigo-400 hover:text-indigo-300">{{ __('auth_signin_link') }}</a>
        </p>
